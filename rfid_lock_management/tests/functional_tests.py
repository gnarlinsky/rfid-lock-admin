...
class GeneralFunctionalTests(LiveServerTestCase):
    ...
    def test_keycard_deactivation_but_some_doors_permitted(self):
        ...
        print colored("\tOpening browser to get to lockuser's change_form.......", "cyan")
        lockuser_id = 1  
        self.browser.get(self.live_server_url + '/lockadmin/rfid_lock_management/lockuser/%d' % lockuser_id)   # in fixture, lock user 1  assigned a keycard and is permitted Space 1 and Space 4 access; the logged in staff user is not permitted access to either of those
        self.browser.maximize_window()
