...
class GeneralFunctionalTests(LiveServerTestCase):
    ...
    def test_no_delete_button_lockuser_change_form(self):
        ...
        when *adding* a LockUser there would not be a 'delete' link/button in any case.) 
        """
        object_id=2
        self.browser.get(self.live_server_url + '/lockadmin/rfid_lock_management/lockuser/%d' % object_id)
        self.browser.maximize_window()
    ...
    def test_keycard_deactivation_but_some_doors_permitted(self):
        ...
        print colored("\tOpening browser to get to lockuser's change_form.......", "cyan")
        lockuser_id = 1  
        # in fixture, lock user 1  assigned a keycard and is permitted Space 1 and Space 4 access; 
        #   the logged in staff user is not permitted access to either of those.
        self.browser.get(self.live_server_url + '/lockadmin/rfid_lock_management/lockuser/%d' % lockuser_id)   
        self.browser.maximize_window()
        ...
        # Note: above throws WebDriverException: Message: u'Compound class names not permitted' 
        current_rfid_field = self.browser.find_element_by_css_selector('.form-row.field-prettify_get_current_rfid')
        current_rfid_field = current_rfid_field.find_element_by_tag_name('p')
        ...
        # Stripping out whitespace in case template formatting introduces extra white space
        ...
        # message 2: a key difference between this test and test_keycard_deactivation
    ...
    def test_keycard_deactivation(self):
        ...
        print colored("\tOpening browser to get to lockuser's change_form.......", "cyan")
        lockuser_id = 3
        self.browser.get(self.live_server_url + '/lockadmin/rfid_lock_management/lockuser/%d' % lockuser_id)   # in fixture, lock user 2  assigned a keycard
        self.browser.maximize_window()
...
class LogIn(LiveServerTestCase):
    ...
    def test_can_do_stuff_in_browser(self):
        ...
        # todo: change
        print colored("\tDoes user see main links in the body?","blue")
        base_page_body = self.browser.find_element_by_tag_name('body')
